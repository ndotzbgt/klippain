...
This is useful for avoiding brush strokes that would cause the toolhead to crash into something or otherwise cause undesired effects. 
One example (and the impetus for this feature) is the case in which one has a brush and a gantry-mounted depressor for a filament cutter on the same side of the gantry. If nozzle cleaning is performed with filament loaded and the brush strokes occur in the same space as the depressor, then the filament cutter is actuated (repeatedly) which results in unnecessary wear and unwanted filament cutting.
...
The brush size is provided to accommodate different brush sizes and is used to calculate the brush stroke distance.
